...
import lang.ExtIndexOutOfBoundsException;

import jtopas.spi.WhitespaceHandler;
import jtopas.spi.KeywordHandler;
import jtopas.spi.PatternHandler;
import jtopas.spi.SeparatorHandler;
import jtopas.spi.SequenceHandler;

import jtopas.spi.StandardWhitespaceHandler;
import jtopas.spi.StandardKeywordHandler;
import jtopas.spi.StandardSeparatorHandler;
import jtopas.spi.StandardSequenceHandler;

import jtopas.spi.DataProvider;
import jtopas.spi.DataMapper;
...
public class StandardTokenizer implements Tokenizer, TokenizerPropertyListener {
   ...
   *                to remove it
   */
  public void setKeywordHandler(jtopas.spi.KeywordHandler handler) {
   ...
   */
  public jtopas.spi.KeywordHandler getKeywordHandler() {
   ...
   */
  public void setWhitespaceHandler(jtopas.spi.WhitespaceHandler handler) {
   ...
   */
  public jtopas.spi.WhitespaceHandler getWhitespaceHandler() {
   ...
   */
  public void setSeparatorHandler(jtopas.spi.SeparatorHandler handler) {
   ...
   */
  public jtopas.spi.SeparatorHandler getSeparatorHandler() {
   ...
   */
  public void setSequenceHandler(jtopas.spi.SequenceHandler handler) {
   ...
   * that parsing is done by the base method of {@link util.AbstractTokenizer}.
   *
   * @return  the currently active {@link SequenceHandler} or null, if the base
   *          implementation is working
   */
  public jtopas.spi.SequenceHandler getSequenceHandler() {
    return _sequenceHandler;
  }
  

  /**
   * Setting a new {@link jtopas.spi.PatternHandler} or removing any 
   * previously installed one. If <code>null</code> is passed, pattern are not 
   * supported by the tokenizer (any longer).
   *
   * @param handler the (new) {@link jtopas.spi.PatternHandler} to 
   *                use or <code>null</code> to remove it
   * @see #getPatternHandler
   */
  public void setPatternHandler(jtopas.spi.PatternHandler handler) {
    _patternHandler = handler;
  }
  
  /**
   * Retrieving the current {@link jtopas.spi.PatternHandler}. The method 
   * may return <code>null</code> if there isn't any handler installed.
   *
   * @return  the currently active {@link jtopas.spi.PatternHandler} 
   *          or <code>null</code>, if patterns are not recognized by the tokenizer
   * @see #setPatternHandler
   */
  public jtopas.spi.PatternHandler getPatternHandler() {
...
__MAIN_LOOP__:
   ...
   * @throws TokenizerException failure while reading data from the input stream
   *         or routed exception from the active {@link jtopas.spi.SequenceHandler}
   ...
   * @throws  TokenizerException routed exception from the active {@link jtopas.spi.KeywordHandler}
   ...
   * Whitespace handler
   */
  private jtopas.spi.WhitespaceHandler _whitespaceHandler = null;

  /**
   * Separator handler
   */
  private jtopas.spi.SeparatorHandler _separatorHandler = null;

  /**
   * Keyword handler
   */
  private jtopas.spi.KeywordHandler _keywordHandler = null;

  /**
   * Sequence handler
   */
  private jtopas.spi.SequenceHandler _sequenceHandler = null;
  
  /**
   * Sequence handler
   */
  private jtopas.spi.PatternHandler _patternHandler = null;
...
/**
 * Implementation of the {@link jtopas.spi.DataProvider} interface 
...
class StandardDataProvider implements DataProvider {
   ...
   * {@link jtopas.Tokenizer}. It should therefore treat it as a 
   * read-only object.
   *<br>
  ...
  /**
   * This method copies the current data range of the {@link jtopas.Tokenizer}
